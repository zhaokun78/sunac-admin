	...
	<div>
		...
		<div class="container padding_b_0">
			<div class="dflex_wrap">
				<div class="dflex_vertical_c margin_r_40 margin_b_20">
					<div class="search_name">导航名称：</div>
					<el-input v-model="req.name" placeholder="请输入" @keyup.enter.native="loadData" @input="loadData"
				...
				<div class="dflex_vertical_c margin_r_40 margin_b_20" v-if="advancedSearch">
					...
					<el-input v-model="req.url" placeholder="请输入" @keyup.enter.native="loadData" @input="loadData"
						class="search_input"></el-input>
				</div>
				<div class="dflex_vertical_c margin_r_40 margin_b_20" v-if="advancedSearch">
		...
		<div class="container use-table">
			...
			<el-table :height="tblHeight" :data="tableDatas" @selection-change="selectionChange" highlight-current-row>
				...
				<el-table-column property="name" label="导航名称" align="center"></el-table-column>
				<el-table-column label="分类图片" align="center">
					<template slot-scope="scope">
						<el-image style="width: 70px;" :src="scope.row.img" :preview-src-list="scope.row.imgs"
...
<script>
	...
	export default {
		...
		methods: {
			...
			toAdd() {
				uni.navigateTo({
					url: `/pages/shop/classify/classify_navigation_add_edit?tab=添加分类`,
					events: {
						refreshData: () => {
							this.loadData();
						}
			...
			toEdit(id) {
				uni.navigateTo({
					url: `/pages/shop/classify/classify_navigation_add_edit?id=${id}&tab=编辑分类`,
					events: {
						refreshData: () => {
							this.loadData();
						}
